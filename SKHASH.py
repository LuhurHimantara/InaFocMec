...
appropriate papers.
	Skoumal, R.J., Hardebeck, J.L., & Shearer, P.M. (2024). SKHASH: A Python
		...
		95(4), 2519-2526. https://doi.org/10.1785/0220230329
...
p_dict={
	...
	'input_format_dlpfile':'', # deep learning P-polarities file format
	...
	'dlpfile':'', # deep learning P-polarities input filename
	...
	'maxout':500, # the maximum number of acceptable focal mechansims considered (e.g., 500).
	'ratmin':3.0, # minimum allowed signal-to-noise ratio
	'badfrac':0.1, # fraction polarities assumed bad
	'badmin':2.0, # minimum number of polarities per event assumed bad. If badmin > [(sum of the polarity weights) * badfrac], badmin replaces badfrac
	'qbadfrac':0.3, # assumed noise in amplitude ratios, log10 (e.g. 0.3 for a factor of 2)
	'qbadmin':2.0, # minimum S/P amplitude misfit allowed per event. If qbadmin > [(sum of the number of S/P measurements) * qbadfrac], qbadmin replaces qbadfrac
	...
	'delmax':0.0, # maximum allowed source-station distance in km. If delmax<=0, uses all results <= than look_del[1] distance.
	...
	'max_agap':270.0, # maximum azimuthal gap
	'max_pgap':90.0, # maximum "plungal" gap
	...
	'flip_takeoff':False, # If True, takeoff angles read/written will correspond to [180: upgoing and 0: downgoing]. If False, it will follow HASH notation [0: upgoing, 180: downgoing].
	...
	'remove_duplicate_stations':False, # If an event_id has multiple polarities or S/P ratios from the same receiver, removes the duplicates
	...
	'ignore_missing_metadata':True, # If True, any measurements with missing metadata will be ignored. If False, an error will be raised.

	'use_fortran':True, # If True, it will attempt to load the Fortran subroutine. If it cannot load or is False, the Python routine will be used.
	...
	'nx0':141, # maximum source-station distance bins for look-up tables
	'nd0':30, # maximum source depth bins for look-up tables
	'look_dep':[0,86,3], # minimum source depth, maximum, and interval for the lookup table
	'look_del':[0,700,5], # minimum source-station distance, maximum, and interval for the lookup table
	...
	'default_takeoff_uncert':0.0, # If the precomputed takeoff uncertainty is missing or negative, this value is used instead when perturbing takeoffs
	'default_azimuth_uncert':0.0, # If the precomputed azimuth uncertainty is missing or negative, this value is used instead when perturbing azimuths

	'plot_station_names':True, # If True, station names will be included in the beachball plot
	'plot_acceptable_solutions':False, # If True, all acceptable solutions will be plotted in the beachball plot
	'plot_mult_solutions':False, # If True and multiple solutions are found, all solutions will be plotted in the beachball plot
	...
	'merge_on':['network', 'station', 'location', 'channel'] # Controls how to merge metadata with observations. List that includes ('network', 'station', 'location', 'channel')
}
...
qual_criteria_dict={
				...
				'azimuthal_gap':90, # max azimuthal gap
				'takeoff_gap':60, # max takeoff gap
				'num_p_pol':8, # minimum number of p polarities
				}
...
if __name__ == "__main__":
	...
	if not((sys.version_info.major==3) & (sys.version_info.minor>=8)):
		print('***WARNING: SKHASH has only been tested on Python 3.8+. Refer to the manual for supported versions.')
	...
	if p_dict['conpfile']:
		...
		if len(tmp_cat_df):
			cat_df.append(tmp_cat_df)
	if p_dict['dlpfile']:
		tmp_cat_df,tmp_pol_df=in_pol.read_polarity_file(p_dict['dlpfile'],p_dict['input_format_dlpfile'],p_dict['merge_on'])
		tmp_pol_df['source']='dl_p'
		if len(tmp_pol_df):
			pol_df.append(tmp_pol_df)
		if len(tmp_cat_df):
			cat_df.append(tmp_cat_df)
	...
	# Looks for duplicate polarity and S/P measurements
	if not(p_dict['allow_duplicate_stations']):
		if len(pol_df):
			if 'sp_ratio' in pol_df.columns:
				dup_df=pd.concat([pol_df[['event_id','sta_code']],pd.isnull(pol_df[['p_polarity','sp_ratio']])],axis=1)
			else:
				dup_df=pd.concat([pol_df[['event_id','sta_code']],pd.isnull(pol_df[['p_polarity']])],axis=1)
			duplicate_polarity_index=pol_df[dup_df.duplicated()].index
			if not(duplicate_polarity_index.empty):
				if p_dict['remove_duplicate_stations']:
					print('Removing duplicate information for {} measurements. Example issues:\n{}'.
						format(len(duplicate_polarity_index),pol_df.loc[duplicate_polarity_index,['event_id','sta_code']].head(10).to_string()))
					pol_df=pol_df.drop(duplicate_polarity_index).reset_index(drop=True)
				else:
					raise ValueError('Duplicate information for {} measurements. Example issues:\n{}'.
						format(len(duplicate_polarity_index),pol_df.loc[duplicate_polarity_index,['event_id','sta_code']].head(10).to_string()))

	if len(pol_df)==0:
		print('No polarity information provided. Exiting.')
		quit()
	...
	'''
	if p_dict['catfile']:
		cat_df=in_other.read_catalog_file(p_dict)

	# Adds event times, locations, and uncertainties to polarity information
	if len(cat_df):
		if not('event_id2' in pol_df.columns):
			pol_df=pol_df.merge(cat_df,on='event_id',how='left')
		else: # composite mechs
			pol_df=pol_df.merge(cat_df.rename(columns={'event_id':'event_id2'}),on='event_id2',how='left')
	...
	'''
	If a station file is given but takeoff and azimuths are already provided, ignore the station file.
	'''
	if p_dict['stfile']:
		if (~(pd.isnull(pol_df[['takeoff','azimuth']]))).any(axis=None):
			print('Ignoring the stfile ({}) because takeoff and azimuths are already provided.'.format(p_dict['stfile']))
			p_dict['stfile']=''
		...
		for event_x,event_id in enumerate(event_ids):
			...
			if mech_dict['mech_qual']:
				...
				if (p_dict['stfile']) and (p_dict['outfile_pol_info']):
					...
					pol_df.loc[mech_dict['event_index'],'takeoff_uncertainty']=mech_dict['takeoff_uncertainty']
					pol_df.loc[mech_dict['event_index'],'azimuth_uncertainty']=mech_dict['azimuth_uncertainty']
	...
	if p_dict['flip_takeoff']:
		pol_df['takeoff']=180-pol_df['takeoff']
